...
class Cart
{
     ...
     * @return string
     */
    public function addone($id, $name = null, $price = null, array $attributes = []){
    	$cart = $this->getCart();
    	
    	$this->event->fire('cart.adding', [$attributes, $cart]);
    	
    	$row = $this->addRow($id, $name, 1, $price, $attributes);
    ...
    public function minusone($rawId){
    	...
    	$raw = $this->updateQty($rawId, $row->qty-1);
